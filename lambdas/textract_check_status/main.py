...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.error(f"Invalid input event. Expected {{'JobId': '...'}}: {str(e)}")
    ...
    try:
        ...
        logger.error(f"Failed to check status for Textract job {job_id}: {str(e)}")
        # Let the Step Function handle this as a retryable error
        raise RuntimeError(f"Textract GetJobStatus failed: {str(e)}")
